...
function amtoolCheckConfig {
  echo "rules: info: checking if Alertmanager config files are valid or not"
  checkconfigOut=$(amtool check-config ${promFiles} ${*} 2>&1)
  checkconfigExitCode=${?}

  # Exit code of 0 indicates success. Print the output and exit.
  if [ ${checkconfigExitCode} -eq 0 ]; then
    echo "checkconfig: info: Alertmanager config files ${promFiles} are valid."
    echo "${checkconfigOut}"
    echo
  ...
  if [ ${checkconfigExitCode} -ne 0 ]; then
    echo "checkconfig: error: Alertmanager config files ${promFiles} are invalid."
    echo "${checkconfigOut}"
    echo
